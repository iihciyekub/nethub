...
test('update check reports a newer release without installing', async () => {
  ...
  const result = await updateNethub(
    ...
    { fetchApi: async () => response('v0.0.6'), execFileApi: async () => { installed = true; } },
  );
  assert.equal(result.latestVersion, '0.0.6');
  assert.equal(installed, false);
  assert.match(writes.join(''), /latest release: 0\.0\.6/);
});
...
test('update installs the tagged GitHub archive globally', async () => {
  ...
  await updateNethub(
    { stdout: { write: () => {} } },
    { fetchApi: async () => response('v0.0.6'), execFileApi: async (...args) => { invocation = args; } },
  );
  assert.deepEqual(invocation[1], [
    'install', '--global', 'https://github.com/iihciyekub/nethub/archive/refs/tags/v0.0.6.tar.gz',
  ]);
});
