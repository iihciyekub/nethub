...
function resolveSettings(cli, config, env = process.env, cwd = process.cwd()) {
  ...
  return {
    ...
    retries: rangedInteger(first(cli.retries, cli.fast ? 0 : undefined, env.NETHUB_RETRIES, config.retries, DEFAULTS.retries), 'retries', 0, 2),
    timeout: integer(first(cli.timeout, cli.fast ? 3000 : undefined, env.NETHUB_TIMEOUT, config.timeout, DEFAULTS.timeout), 'timeout', 1),
    linkTimeout: integer(first(cli.linkTimeout, cli.fast ? 500 : undefined, env.NETHUB_LINK_TIMEOUT, config.linkTimeout, DEFAULTS.linkTimeout), 'link timeout', 1),
